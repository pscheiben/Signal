...
classdef Signal
    ...
    methods
        ...
        function Y = generate(sig)                   %Data_Vector generator
            Phase_Rad = sig.Phase*2*pi/360;
            Sampling_Frequency = sig.Frequency*sig.Sample_per_Cycle;
            End_Time = sig.Start_Time + sig.Duration;
            time=[sig.Start_Time:(1/Sampling_Frequency):End_Time];
            Y = sig.Amplitude*cos(2*pi*sig.Frequency*(time)+Phase_Rad);
        end
        ...
        function plotaround(sig,Ref_Point,Range)
            if (Ref_Point<sig.Start_Time || Ref_Point>(sig.Start_Time+sig.Duration))            
                ...
                range_idx = Range*sig.Sample_per_Cycle
